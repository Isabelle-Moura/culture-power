...
import { fakeAdminRepository } from "../_mocks/fake.admim.repository";
...
describe("AdminService", () => {
  ...
  describe("findAdminEmail", () => {
    it("Should return an admin when finding admin email", async () => {
      const result = await adminService.findAdminEmail(fakeAdmin.email);
      expect(result).toEqual(fakeAdmin);
    })
    
    it("Should handle admin not found when finding admin email", async () => {
      vi.spyOn(fakeAdminRepository, "findAdminEmail").mockRejectedValue(new Error("Admin not found"));
      await expect(adminService.findAdminEmail("nonexistentAdminEmail")).rejects.toThrowError("Admin not found");
    })
  })
});
