...
describe("ProductService", () => {
  ...
  describe("findById", async () => {
    ...
    it("Should return an error if no product is found.", async () => {
      vi.spyOn(fakeProductRepository, "findById").mockImplementationOnce(() =>
  ...
  describe("createProduct", async () => {
    ...
    it("Should return an error if not able to create product.", async () => {
      vi.spyOn(fakeProductRepository, "createProduct").mockImplementationOnce(
        () => Promise.resolve(null)
      );
      await expect(
  ...
  describe("updateProduct", async () => {
    ...
    it("Should return an error if not able to update product.", async () => {
      vi.spyOn(fakeProductRepository, "updateProduct").mockImplementationOnce(
        () => Promise.resolve(null)
      );
      await expect(
  ...
  describe("redeemProduct", async () => {
    it("Should return an product redeemed.", async () => {
      const redeemedProduct = await productService.redeemProduct(
        fakeUser._id,
        fakeProduct._id
      );
    ...
    it("Should return an error if not able to redeem product.", async () => {
      vi.spyOn(fakeProductRepository, "redeemProduct").mockImplementationOnce(
        () => Promise.resolve(null)
      );
      await expect(
        productService.redeemProduct(fakeUser._id, fakeProduct._id)
      ).rejects.toThrow();
    });
  });
});
