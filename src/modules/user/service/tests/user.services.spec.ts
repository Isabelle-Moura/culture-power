...
describe("UserService", () => {
  ...
  describe("getAll", () => {
    ...
    it("Should return an error if no user is found.", async () => {
      vi.spyOn(fakeUserRepository, "getAll").mockImplementationOnce(() =>
        Promise.reject([])
      );
  ...
  describe("findByEmail", () => {
    ...
    it("Should return an error if no user is found.", async () => {
      vi.spyOn(fakeUserRepository, "findByEmail").mockImplementationOnce(() =>
  ...
  describe("createUser", () => {
    it.skip("Should return an user.", async () => {
      //FIXME: Is returning the message error: "User already exists." and i don't know why.
    ...
    it("Should return an error if not able to create user.", async () => {
      vi.spyOn(fakeUserRepository, "createUser").mockImplementationOnce(() =>
        Promise.resolve(null)
      );
      await expect(
        userService.createUser(fakeUser, fakeUser.photo)
      ).rejects.toThrow();
    });
  });
});
